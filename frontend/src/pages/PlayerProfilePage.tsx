...
export function PlayerProfilePage() {
    ...
    const reportRef = useRef<HTMLDivElement>(null); // Referencia para el PDF completo
    ...
    const handleDownloadExcel = () => {
        if (player) {
            exportToExcel([player], `ficha_${player.lastName}`);
        }
    };

    const handleDownloadPDF = () => {
        if (reportRef.current && player) {
            exportToPDF(reportRef.current, `reporte_${player.lastName}`);
        }
    };

    if (isLoading) {
        return <div style={{ textAlign: "center", padding: 80, color: C.muted }}>Cargando perfil...</div>;
    }
    ...
    const statCircles = percentiles && latestStats ? [
        { label: "Remates al Arco", value: latestStats.shotsOnTarget, pct: percentiles.shotsOnTarget },
        { label: "Conversión", value: latestStats.goals, pct: percentiles.goals },
        { label: "Duelos Ofensivos", value: latestStats.aerialDuelsWon, pct: percentiles.aerialDuelsWon },
        { label: "Duelos Aéreos", value: latestStats.aerialDuelsWon, pct: percentiles.aerialDuelsWon },
    ] : [];
    ...
    return (
        <div style={{ fontFamily: "'Nunito Sans', sans-serif", paddingBottom: 80 }}>
            ...
            {/* 👇 TODO EL CONTENIDO DEL REPORTE ENVUELTO EN LA REF */}
            <div ref={reportRef} style={{ background: C.bg }}>
                <div
                    ref={heroRef}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap", position: "relative", zIndex: 1 }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6, fontSize: 13, flexWrap: "wrap" }}>
                                <span style={{ background: "rgba(0,224,148,0.12)", color: C.primary, padding: "2px 8px", borderRadius: 99, fontWeight: 700, fontSize: 12 }}>
                                    {player.position}
                                </span>
                ...
                <div style={{
                    ...
                    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24 }}>
                        ...
                        {latestStats ? (
                            ...
                            <div style={{ color: C.muted, fontSize: 13 }}>Sin estadísticas disponibles</div>
                        )}
                    </div>
                </div>

                {/* ── RADAR & HEATMAP ── */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 400px), 1fr))", gap: 24, marginBottom: 24 }}>
                    ...
                    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, display: "flex", flexDirection: "column" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "center", flex: 1, alignItems: "center", width: "100%" }}>
                ...
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
                    ...
                    {latestStats && (
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 24, padding: "16px 0" }}>
